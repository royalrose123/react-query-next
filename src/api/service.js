...
class Service {
  ...
  getAccessToken() {
    if (this.currentEnv === 'local') return null
  ...
  getApiConfig() {
    ...
    return { baseURL: 'http://inter-apitw.tutorabc.com/tmdapi/api' }
  }
  ...
  handleResponse(response) {
    ...
    console.log(`casedData`, casedData)

    const { returnCode, returnMessage, returnDetail, success } = casedData

    // TODO: 還要確定 return code
    // const isRequestSuccess = returnCode === '0000'
    const isRequestSuccess = success

    if (isRequestSuccess) {
      return this.handleSuccess(casedData.result)
    }
